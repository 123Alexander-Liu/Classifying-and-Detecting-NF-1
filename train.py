...
from losses import LovaszLossSoftmax  #mutiplue classification
from losses import LovaszLossHinge # two classification
from losses import dice_coeff # two classification
...
def train_net(net, cfg):
    ...
    train = BasicDataset(cfg.train_images_dir, cfg.train_masks_dir, cfg.scale)
    val  = BasicDataset(cfg.val_images_dir, cfg.val_masks_dir, cfg.scale)
    ...
    if cfg.n_classes > 1:
        criterion = LovaszLossSoftmax()  #LovaszLossSoftmax loss from 2018
    else:
    ...
    for epoch in range(cfg.epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{cfg.epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                inference_masks = net(batch_imgs)[0]
                ...
                if cfg.deepsupervision: #采用深度监督学习，则将输入前向传导得到分支和最后层的损失，共4张特征图，均做反向传播
                    loss = 0
            ...
            if True:
                ...
                """
                if global_step % (len(dataset) // ( cfg.batch_size)) == 0:
                    val_score = eval_net(net, val_loader, device, n_val, cfg)
                    if cfg.n_classes > 1:
                        logging.info('Validation cross entropy: {}'.format(val_score))
