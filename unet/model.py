...
class NestedUNet(nn.Module): 
    def __init__(self, cfg):
        ...
        #self.up = nn.Upsample(scale_factor=2, mode='nearest', align_corners=None)  #为了画图
    ...
    def forward(self, input):
        ...
        x0_0 = self.conv0_0(input) # n 32 512 512 
        x1_0 = self.conv1_0(self.pool(x0_0)) #32 256 256 -> 64 256 256 
        x0_1 = self.conv0_1(torch.cat([x0_0, self.up(x1_0)], 1))  #-> 512 ,512 

        x2_0 = self.conv2_0(self.pool(x1_0)) #64 256 256-> 128 128,128
        x1_1 = self.conv1_1(torch.cat([x1_0, self.up(x2_0)], 1))
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, self.up(x1_1)], 1))

        x3_0 = self.conv3_0(self.pool(x2_0))  # -> 256 , 64,64 
        x2_1 = self.conv2_1(torch.cat([x2_0, self.up(x3_0)], 1))
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1))

        x4_0 = self.conv4_0(self.pool(x3_0)) #-> 512 , 32,32 
        x3_1 = self.conv3_1(torch.cat([x3_0, self.up(x4_0)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up(x3_1)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2)], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3)], 1)) # 512
        ...
        if self.deepsupervision:
            ...
            return [output1, output2, output3, output4],x0_4
